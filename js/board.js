...
function generateMenuItems(task) {
  ...
  for (let i = 0; i < statusArr.length; i++) {
    ...
    if (task.status !== category) {
      ...
      menuItems += `<a class="MenuButton" href="#${category}-headline" onclick="${moveToFunction}">${category.charAt(0).toUpperCase() + category.slice(1)
        }</a>`;
    }
  }
  return menuItems;
 ...
 * If there are more than 3 users assigned, only the first three are rendered plus one additional badge indicating
...
function renderAssignedBadges() {
  for (let i = 0; i < filteredTasks.length; i++) {
    ...
    badge.innerHTML = "";
    const numberOfBadges = Math.min(assignedContacts.length, 3);
    const additionalAssignees = Math.max(0, assignedContacts.length - 3);
    for (let j = 0; j < numberOfBadges; j++) {
      const assignedContact = assignedContacts[j];
      badge.innerHTML += generateBadgeHTML(assignedContact);
    }
    if (additionalAssignees > 0) badge.innerHTML += generateBadgeHTML({ color: 'gray', initials: "+" + additionalAssignees });
  }
}
 ...
 * @param {number} id - Id of card being dragged.
 */

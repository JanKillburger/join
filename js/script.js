...
window.onresize = function () {
  if (getScreenType() != loaded && document.querySelector('div[include-templates]') != null) {
    getTemplates();
  }
};
...
/**
 * Unloads the not needed Template.
...
function markCorrectMenuPoint() {
  ...
  try {
    ...
    if (toChoose) {
      ...
      if (helpIcon != null) helpIcon.classList.add("d-none");
    }
  } catch {
    return;
  }
...
function hideMenusDesktop() {
  ...
  if (siderMenu != null) siderMenu.style.display = "none";
  let siderBlocker = document.getElementById("sider_blocker");
  if (siderBlocker != null) siderBlocker.classList.remove("d-none");
...
function toLegal() {
  saveDocName();
  window.open("./legal_notes.html", '_self');
}
...
/**
 * Shows fly in notification and forwards to next page.
 * @param {HTMLElement} elementId - Notification element.
 * @param {*} targetHref - Html page to forward to.
 */
...
function renderContactBubbleHtml(contact) {
  return /*html*/ `<svg
    ...
    class="contact-bubble-${contact.id} contact-bubble"
  >
    <circle
      cx="21"
      cy="21"
